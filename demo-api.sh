...
curl -X POST ${BASE_URL}/command \
  ...
  -d '{"type":"MOVE","armyIndex":0,"targetX":3,"targetY":3}' \
  -s > /dev/null
echo "   Done!"
echo ""

echo "3. Advancing game by 1 tick (army on village gains 1 soldier)..."
curl -X POST ${BASE_URL}/tick -s | python3 -c "import sys, json; data=json.load(sys.stdin); print(f\"   Tick: {data['tickCount']}, Army 0 soldiers: {data['armies'][0]['soldiers']}\")"
echo ""

echo "4. Moving player 2's army to same location for combat..."
curl -X POST ${BASE_URL}/command \
  -H "Content-Type: application/json" \
  -d '{"type":"MOVE","armyIndex":1,"targetX":3,"targetY":3}' \
  -s > /dev/null
